<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Dwaipayan%20Dutta&fontSize=48&fontColor=fff&animation=twinkling&fontAlignY=36&desc=Senior%20AI%20Engineer%20%7C%20GenAI%20%7C%20Agentic%20AI%20%7C%20MLOps&descAlignY=58&descSize=17" width="100%"/>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&duration=2800&pause=800&color=58A6FF&center=true&vCenter=true&multiline=false&repeat=true&width=640&lines=Building+Agentic+AI+Systems+at+Scale;LangGraph+%7C+LangChain+%7C+CrewAI+%7C+AutoGen;Multi-Agent+Orchestration+%26+RAG+Pipelines;AI+Platform+Architect+%7C+MLOps+Engineer;Enterprise+GenAI+%7C+LLM+Applications)](https://git.io/typing-svg)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=DwaipayanDutta&label=Profile+Views&color=0e75b6&style=flat)
&nbsp;
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/dwaipayan-dutta-a39292129)
&nbsp;
[![Email](https://img.shields.io/badge/Gmail-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:duttdw01@gmail.com)
&nbsp;
![Nagarro](https://img.shields.io/badge/Nagarro-Senior%20AI%20Engineer-FF6B35?style=flat&logoColor=white)

</div>

---

## About Me

```python
class DwaipayanDutta:

    role        = "Senior AI Engineer | GenAI Engineer | AI Platform Architect"
    company     = "Nagarro"
    location    = "Bangalore, India"

    focus       = [
        "Agentic AI Platforms & Multi-Agent Systems",
        "LangGraph · LangChain · CrewAI · AutoGen",
        "RAG Pipelines & Knowledge Management Systems",
        "LLM Applications & Enterprise GenAI",
        "MLOps · AI Automation · Cloud Deployments",
    ]

    philosophy  = "If it works → understand why. If it fails → understand faster."

    contact     = "duttdw01@gmail.com"
```

I design and ship **production-grade AI systems** — from multi-agent orchestration pipelines to enterprise RAG architectures. My work sits at the intersection of applied AI research and scalable engineering: I care as much about system design as I do about model quality.

Currently leading development of **Agentic AI Platforms** with human-in-the-loop workflows, building on top of LangGraph and deploying on AWS. Background spans Quantitative Analytics, Computational Finance, and full-stack ML — from feature engineering to inference serving.

---

## Current Focus

| Area | What I'm Building |
|---|---|
| **Agentic AI** | Multi-agent systems with LangGraph, human-in-the-loop orchestration, tool-using agents |
| **RAG & Knowledge** | Production RAG pipelines, vector stores (ChromaDB, pgvector), hybrid retrieval |
| **LLM Engineering** | Prompt engineering, fine-tuning, evaluation frameworks, LLM-as-judge pipelines |
| **MLOps** | ML workflow orchestration with Airflow, experiment tracking with MLflow, CI/CD for models |
| **Cloud AI** | AWS SageMaker, Lambda, ECS deployments; Azure AI services integration |

---

## Tech Stack

**AI / LLM**

[![Python](https://skillicons.dev/icons?i=python&theme=dark)](https://python.org)
&nbsp;
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=flat&logo=langchain&logoColor=white)
![CrewAI](https://img.shields.io/badge/CrewAI-FF3366?style=flat&logoColor=white)
![AutoGen](https://img.shields.io/badge/AutoGen-0078D4?style=flat&logo=microsoft&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![Claude](https://img.shields.io/badge/Anthropic%20Claude-CC785C?style=flat&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)

**Machine Learning**

[![ML Stack](https://skillicons.dev/icons?i=tensorflow,pytorch,sklearn&theme=dark)](https://skillicons.dev)
&nbsp;
![Transformers](https://img.shields.io/badge/Transformers-FFD21E?style=flat&logo=huggingface&logoColor=black)
![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat&logo=mlflow&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Spark](https://img.shields.io/badge/Apache%20Spark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat&logo=apacheairflow&logoColor=white)

**Languages**

[![Languages](https://skillicons.dev/icons?i=python,js,r&theme=dark)](https://skillicons.dev)
&nbsp;
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![SAS](https://img.shields.io/badge/SAS-003865?style=flat&logoColor=white)

**Backend & APIs**

[![Backend](https://skillicons.dev/icons?i=fastapi,flask&theme=dark)](https://skillicons.dev)
&nbsp;
![REST](https://img.shields.io/badge/REST%20APIs-009688?style=flat&logo=fastapi&logoColor=white)

**Databases & Vector Stores**

[![Databases](https://skillicons.dev/icons?i=postgres,mysql,redis,dynamodb&theme=dark)](https://skillicons.dev)
&nbsp;
![ChromaDB](https://img.shields.io/badge/ChromaDB-FF6B35?style=flat&logoColor=white)
![pgvector](https://img.shields.io/badge/pgvector-336791?style=flat&logo=postgresql&logoColor=white)

**Cloud & DevOps**

[![Cloud](https://skillicons.dev/icons?i=aws,azure,docker,kubernetes,githubactions,nginx&theme=dark)](https://skillicons.dev)

---

## Architecture Expertise

```
Agentic AI Systems
├── Multi-Agent Orchestration (LangGraph, CrewAI, AutoGen)
├── Human-in-the-Loop Workflows
├── Tool-Using Agents & Function Calling
├── Agent Memory & State Management
└── Supervisor / Hierarchical Agent Patterns

RAG & Knowledge Management
├── Hybrid Retrieval (semantic + keyword)
├── Vector Stores (ChromaDB, pgvector, FAISS)
├── Document Parsing & Chunking Strategies
├── Re-ranking & Contextual Compression
└── Evaluation: RAGAS, LLM-as-judge

LLM Engineering
├── Prompt Engineering & Chain-of-Thought
├── Fine-tuning (LoRA, QLoRA)
├── LLM Evaluation Frameworks
├── Structured Output / JSON Mode
└── Streaming Inference & Latency Optimization

MLOps & AI Platform
├── ML Pipeline Orchestration (Airflow, Step Functions)
├── Experiment Tracking (MLflow)
├── Model Registry & Versioning
├── Feature Stores
└── CI/CD for ML (GitHub Actions, CodePipeline)

Data Science & Analytics
├── Propensity & Scoring Models
├── Quantitative Analytics
├── Computational Finance
└── Statistical Modeling (R, SAS, Python)
```

---

## Featured Projects

> Public projects coming soon — pinned repositories being updated.

| Project | Stack | Description |
|---|---|---|
| **Agentic AI Platform** | LangGraph · FastAPI · AWS | Multi-agent orchestration with human-in-the-loop approval workflows and tool-calling agents |
| **Enterprise RAG System** | LangChain · ChromaDB · pgvector | Production RAG pipeline with hybrid retrieval, re-ranking, and RAGAS evaluation |
| **Propensity Modeling Engine** | XGBoost · Spark · Airflow | End-to-end ML pipeline for propensity and income estimation at scale |
| **ML Workflow Orchestrator** | Airflow · MLflow · Docker | Automated feature engineering, training, evaluation, and deployment pipeline |

---

## GitHub Statistics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=DwaipayanDutta&show_icons=true&theme=github_dark&hide_border=true&count_private=true&include_all_commits=true" height="170"/>
&nbsp;&nbsp;
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=DwaipayanDutta&layout=compact&theme=github_dark&hide_border=true&langs_count=8" height="170"/>

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=DwaipayanDutta&theme=github-dark-blue&hide_border=true)](https://git.io/streak-stats)

</div>

---

## GitHub Trophies

<div align="center">

[![trophy](https://github-profile-trophy.vercel.app/?username=DwaipayanDutta&theme=darkhub&no-frame=true&no-bg=true&margin-w=4&row=1)](https://github.com/ryo-ma/github-profile-trophy)

</div>

---

## Contribution Activity

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=DwaipayanDutta&theme=github-compact&hide_border=true&area=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## Contribution Snake

<div align="center">

<!-- Snake is generated by GitHub Actions. Run the workflow from the Actions tab first. -->
<!-- Once the `output` branch exists, this image will auto-render. -->
<img src="https://raw.githubusercontent.com/DwaipayanDutta/DwaipayanDutta/output/github-snake-dark.svg" alt="contribution snake animation"/>

</div>

---

## Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Dwaipayan%20Dutta-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/dwaipayan-dutta-a39292129)
&nbsp;
[![Email](https://img.shields.io/badge/Email-duttdw01%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:duttdw01@gmail.com)
&nbsp;
[![GitHub](https://img.shields.io/badge/GitHub-DwaipayanDutta-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/DwaipayanDutta)

</div>

---

## Fun Facts

```
> I treat every model like a black box until I understand what's inside.
> Debugging an agent pipeline is 10% code, 90% existential crisis.
> Latency is a product decision disguised as an engineering problem.
> The best RAG system is the one your users don't notice.
> math > opinions  ·  experiments > assumptions  ·  systems > models
```

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" width="100%"/>
